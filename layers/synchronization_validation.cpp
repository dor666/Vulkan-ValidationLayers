...
std::vector<SyncStageAccessIndex> GetSyncStageAccessIndexsByDescriptorSet(VkDescriptorType descriptor_type,
                                                                          const interface_var &descriptor_data,
    ...
    std::vector<SyncStageAccessIndex> sync_indices;
    if (descriptor_data.is_writable) {
        sync_indices.emplace_back(stage_access->second.shader_write);
    }
    sync_indices.emplace_back(stage_access->second.shader_read);
    return sync_indices;
}
...
bool SyncValidator::ValidateDescriptorSet(const AccessContext &context, const CMD_BUFFER_STATE &cmd,
    ...
    for (const auto &stage_state : pPipe->stage_state) {
        for (const auto &set_binding : stage_state.descriptor_uses) {
            cvdescriptorset::DescriptorSet *descriptor_set = state.per_set[set_binding.first.first].bound_descriptor_set;
            ...
            for (uint32_t i = index_range.start; i < index_range.end; ++i, ++array_idx) {
                ...
                switch (descriptor->GetClass()) {
                    ...
                    case DescriptorClass::Image: {
                        ...
                        for (auto sync_index : sync_indices) {
            ...
            for (uint32_t i = index_range.start; i < index_range.end; ++i, ++array_idx) {
                ...
                switch (descriptor->GetClass()) {
                    ...
                    case DescriptorClass::Image: {
                        ...
                        const IMAGE_STATE *img_state = img_view_state->image_state.get();
                        for (auto sync_index : sync_indices) {
                            context.UpdateAccessState(*img_state, sync_index, img_view_state->normalized_subresource_range,
